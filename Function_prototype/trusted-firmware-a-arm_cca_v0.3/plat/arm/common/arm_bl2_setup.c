...
static void arm_bl2_plat_gpt_setup(void)
{
	...
	INFO("Enabling Granule Protection Table\n");
	if (gpt_init(&gpt_params) < 0) {
		panic();
	}

	pas_region_t pas_regions_2[] = {
		ARM_PAS_GPI_ANY,
		ARM_PAS_KERNEL_NS,
		ARM_PAS_KERNEL_INS,
		ARM_PAS_KERNEL_NS_2,
	...
	pas_region_t pas_regions_3[] = {
		ARM_PAS_GPI_ANY,
		ARM_PAS_KERNEL_NS,
		ARM_PAS_KERNEL_INS_SEC,
		ARM_PAS_KERNEL_NS_2,
		ARM_PAS_OTHER_ROOTS
	};
	...
	for (int i = 0; i < MAX_CORE_NUM; i++) {
		if (i >= 8) {
			gpt_init_params_t gpt_params_3 = {
				PLATFORM_PGS,
				PLATFORM_PPS,
				PLATFORM_L0GPTSZ,
				pas_regions_3,
				(unsigned int)(sizeof(pas_regions_3)/sizeof(pas_region_t)),
				ARM_GPT_L0_BASE + (i * ARM_L0_GPT_SIZE), ARM_L0_GPT_SIZE,
				ARM_GPT_L1_BASE + (i * (ARM_L1_GPT_SIZE * 2)), ARM_L1_GPT_SIZE
			};
			INFO("Enabling Granule Protection Table for kernel .text\n");
			if (gpt_2_init(&gpt_params_3, i) < 0) {
				panic();
			}
		} else {
			gpt_init_params_t gpt_params2 = {
				...
				PLATFORM_L0GPTSZ,
				pas_regions_2,
				(unsigned int)(sizeof(pas_regions_2)/sizeof(pas_region_t)),
				ARM_GPT_L0_BASE + (i * ARM_L0_GPT_SIZE), ARM_L0_GPT_SIZE,
				ARM_GPT_L1_BASE + (i * (ARM_L1_GPT_SIZE * 2)), ARM_L1_GPT_SIZE
			};

			INFO("Create %d Second Granule Protection Table\n", i);
