...
uint32_t plat_ic_get_mpidr(uint32_t id) {
	uint64_t mpidr_base = 0x0000000081000000;
	...
	return ((id / 4) * 0x10000 + (id % 4) * 0x100) | mpidr_base;
}

uint32_t plat_ic_handle_pmu_interrupt(void) {
	u_register_t mpidr;
	uint32_t interrupt_id, intr_raw;
	//plat_ic_acknowledge_interrupt();


	intr_raw = plat_ic_acknowledge_interrupt();
	interrupt_id = plat_ic_get_interrupt_id(intr_raw);
	if (interrupt_id == 0xe)
	{
    	__asm__ volatile ("msr PMOVSCLR_EL0, %0" : : "r" (0x1));
    	__asm__ volatile ("msr PMEVCNTR0_EL0, %0" : : "r" (0xffffffffffffffff));

		plat_ic_end_of_interrupt(interrupt_id);
		return 1;
	}

	mpidr = plat_ic_get_mpidr(interrupt_id);
	if (mpidr == 0)
	{
		// ERROR("not pmu interrupt or sgi 14 : %d\n", interrupt_id);
		return 0;
	}
	plat_ic_end_of_interrupt(interrupt_id);
	...
	//gicv3_disable_interrupt(intr_raw, plat_core_pos_by_mpidr(mpidr));
	//plat_ic_clear_interrupt_pending(intr_raw);
	//gicv3_clear_interrupt_pending(intr_raw, plat_core_pos_by_mpidr(mpidr));

	if (mpidr == read_mpidr_el1())
	{
    	__asm__ volatile ("msr PMOVSCLR_EL0, %0" : : "r" (0x1));
    	__asm__ volatile ("msr PMEVCNTR0_EL0, %0" : : "r" (0xffffffffffffffff));
	...
	plat_ic_raise_el3_sgi(0xe, mpidr);
	// write_scr_el3(read_scr_el3() | (SCR_FIQ_BIT));
	//plat_ic_end_of_interrupt(intr_raw);
	//plat_ic_enable_interrupt(interrupt_id);
	//enable_intr_rm_local(INTR_TYPE_EL3, SECURE);
	//gicv3_enable_interrupt(intr_raw, plat_core_pos_by_mpidr(mpidr));
	// INFO("interrupt_id %x, intr raw %x, interrupt_id2 %x\n", interrupt_id, intr_raw, interrupt_id2);
	return 1;
}
