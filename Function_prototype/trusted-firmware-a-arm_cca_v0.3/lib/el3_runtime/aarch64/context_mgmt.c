...
void cm_el1_sysregs_context_save(uint32_t security_state)
{
	...
	// NOTICE("TTBR1_EL1 IS :     0x%016lx\n", read_ttbr1_el1());
	// NOTICE("el1 context addr IS :     0x%016llx\n", (uint64_t) get_el1_sysregs_ctx(ctx));
			  ...
			  uint32_t value)
{
	...
	NOTICE("cm write scr el3 : 0x%016lx\n", scr_el3);
	scr_el3 &= ~(1UL << bit_pos);
	scr_el3 |= (u_register_t)value << bit_pos;
	NOTICE("cm write scr el3 after: 0x%016lx\n", scr_el3);
	write_ctx_reg(state, CTX_SCR_EL3, scr_el3);
}
