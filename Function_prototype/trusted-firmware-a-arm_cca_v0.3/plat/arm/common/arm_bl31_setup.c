...
uint64_t phys_va_base;
// int patch_num = 0;
// patch_meta_t patch_lists[MAX_PATCH_NUM];
#endif
...
# endif
	...
	{
		gpt_init_params_t gpt_params2 = {
			PLATFORM_PGS,
			PLATFORM_PPS,
			PLATFORM_L0GPTSZ,
			NULL,
			0U,
			ARM_GPT_L0_BASE + (i * ARM_L0_GPT_SIZE), ARM_L0_GPT_SIZE,
			ARM_GPT_L1_BASE + (i * (ARM_L1_GPT_SIZE * 2)), ARM_L1_GPT_SIZE
		};

		INFO("Create no.2 %d Second Granule Protection Table\n", i);
		if (gpt_2_init(&gpt_params2, i) < 0)
		{
			panic();
		}
	}

	*(uint64_t *)RESERVED_BASE = 0x0;
	*(uint64_t *)PATCH_TRIGER_NUM = 0x0;
	// *(uint64_t *)(RESERVED_BASE + 0x8) = RESERVED_BASE + PATCH_BASE_OFFSET;

	*(uint64_t *)(RESERVED_BASE + DATA_ADDR_BASE_OFFSET) = 0x0;
	*(uint64_t *)(RESERVED_BASE + DATA_ADDR_BASE_OFFSET + 0x8) = RESERVED_BASE + DATA_ADDR_BASE_OFFSET + DATA_BASE_OFFSET;



	// gpt_2_set_pas(PTE_PAGE_1, GPI_SECURE, 8);
	// gpt_2_set_pas(PTE_PAGE_2, GPI_SECURE, 8);
	// gpt_2_set_pas(PTE_PAGE_3, GPI_SECURE, 8);
	// gpt_2_set_pas(0x80083400, GPI_NS);
	// patch_lists[patch_num].base_pa = 0xfe000000;
	// patch_lists[patch_num].size = 0x1000;
	// int result_t = gpt_set_pas(patch_lists[patch_num].base_pa, GPI_ROOT);
	// INFO("Set 0x%016llx to root world success, result number is %d\n", patch_lists[patch_num].base_pa, result_t);
	// patch_num++;
#endif /* ENABLE_RME */
}
